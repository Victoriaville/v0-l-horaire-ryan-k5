...
type SortKey = "created_at" | "requester_date" | "target_date" | "status" | "requester_name" | "target_name" | "duration"
...
export function MyExchangesTab({ exchanges, userId }: MyExchangesTabProps) {
  ...
  const handleCancel = async (exchangeId: number) => {
    if (!confirm("Êtes-vous sûr de vouloir annuler cette demande d'échange?")) return
